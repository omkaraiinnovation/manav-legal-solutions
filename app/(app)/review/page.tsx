...
export default async function ReviewQueuePage() {
  ...
  const draftsRaw = await Drafts.all();
  const drafts = draftsRaw.sort((a, b) => b.createdAt.localeCompare(a.createdAt));
  const mattersById = new Map((await Promise.all(drafts.map(async (d) => [d.matterId, await Matters.get(d.matterId)] as const))));
    ...
    <div>
      ...
      <div className="mx-auto max-w-4xl px-6 py-6">
        ...
        <div className="paper-card divide-y" style={{ borderColor: "var(--hairline)" }}>
          {drafts.map((d) => {
            const matter = mattersById.get(d.matterId);
            return (
