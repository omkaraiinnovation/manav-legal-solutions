...
export default async function DashboardPage() {
  ...
  const [matters, pendingDrafts, allDeadlinesRaw, acts, provisions] = await Promise.all([
    Matters.byTenant(user.tenantId), Drafts.pendingReview(), Deadlines.all(), Acts.all(), Provisions.all(),
  ]);
  const allDeadlines = allDeadlinesRaw.filter((d) => d.status === "pending");
