...
export default async function ActDetailPage({ params }: { params: Promise<{ actId: string }> }) {
  ...
  const provisions = Provisions.byAct(actId).sort((a, b) => a.sectionNumber.localeCompare(b.sectionNumber, undefined, { numeric: true }));
      ...
      <div className="mx-auto max-w-4xl space-y-6 px-6 py-6">
        ...
        <section>
          ...
          <div className="paper-card divide-y" style={{ borderColor: "var(--hairline)" }}>
            {provisions.map((p) => {
              const rels = LegalRelationships.fromNode(p.id);
              return (
