...
export default async function ReviewDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const matter = Matters.get(draft.matterId);
  const documentType = DocumentTypes.get(draft.documentTypeId);
  const citations = DraftCitations.byDraft(id);
  const history = ReviewActions.byDraft(id);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px_280px]" style={{ minHeight: "calc(100vh - 4rem)" }}>
        ...
        <div className="border-r p-6" style={{ borderColor: "var(--hairline)" }}>
          ...
          <div className="space-y-2.5">
            {citations.map((c) => {
              const provision = c.provisionId ? Provisions.get(c.provisionId) : undefined;
              const act = provision ? Acts.get(provision.actId) : undefined;
              const caseLaw = c.caseLawId ? CaseLaws.get(c.caseLawId) : undefined;
              return (
