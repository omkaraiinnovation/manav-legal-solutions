...
export default async function AdminPage() {
  ...
  const users = Users.byTenant(tenant.id);
  const acts = Acts.all();
  const provisions = Provisions.all();
  const auditLog = AuditLogs.all().sort((a, b) => b.createdAt.localeCompare(a.createdAt)).slice(0, 12);
  const statePacks = StateOnboarding.all();
  const matters = Matters.byTenant(tenant.id);
  const documentTypes = DocumentTypes.all();
  ...
  return (
    <div>
      ...
      <div className="mx-auto max-w-5xl space-y-8 px-6 py-6">
        ...
        <section>
          <SectionHeader eyebrow="Multi-Tenant Console" title={tenant.name} action={<ResetDemoDataButton />} />
        ...
        <section>
          ...
          <div className="paper-card divide-y font-mono text-xs" style={{ borderColor: "var(--hairline)" }}>
            ...
            {auditLog.length === 0 && <div className="p-4 text-ink-faint">No audit events yet this session.</div>}
          </div>
        </section>
      </div>
    </div>
